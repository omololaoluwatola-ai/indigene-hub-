...
//
//  - RENDERING (updated): the PDF is NEVER navigated to as a raw file URL
//    anymore. It's fetched once as a blob, cached in the browser's own
//    Cache Storage (origin-scoped, invisible to the phone's Downloads
//    folder), and rendered page-by-page onto <canvas> elements using
//    PDF.js. Later opens read straight from cache — instant, no data
//    used — but there is never a real .pdf file the OS or another app can
//    pick up, save, or reopen. This is what stops the "downloads onto the
//    phone and opens outside the site" bug.
//
// Usage on a PDF page:
...
function readCachedMonetization(ignoreTTL = false) {
  try {
    const raw = localStorage.getItem(MONETIZATION_CACHE_KEY);
    if (!raw) return null;
    const { value, ts } = JSON.parse(raw);
    if (!ignoreTTL && Date.now() - ts > MONETIZATION_CACHE_TTL_MS) return null;
    return value;
  } catch (e) { return null; }
}
...
async function fetchMonetizationActive() {
  try {
    ...
    // Can't reach Firestore right now (e.g. offline). Don't blindly assume
    // dormant — that's what let a lock silently disappear once the normal
    // 5-min cache went stale while offline. Instead fall back to the last
    // value we ever confirmed, no matter how old, same as Firestore's own
    // offline persistence does for the per-PDF access check below. Only a
    // device that has NEVER once confirmed a value falls through to the
    // safe "dormant" default.
    const stale = readCachedMonetization(true);
    return stale === null ? false : stale;
  }
}
...
// ---- PDF rendering (blob + Cache API + PDF.js canvas, never a raw file URL) ----

const PDF_CACHE_NAME = 'indigene-pdf-cache-v1';

async function getCachedOrFetchPdfBlob(url) {
  try {
    ...
    // Cache Storage unsupported or failed for some reason — fall back to a
    // plain fetch. Still rendered on canvas below, never navigated to, so
    // the no-download behavior holds either way.
    const response = await fetch(url);
    return await response.blob();
  }
}

// Blocks Ctrl+S / Cmd+S while a PDF is open in the viewer, on top of the
// per-canvas right-click block added in renderViewer. Registered once.
let saveBlockerAttached = false;
function attachSaveBlocker() {
  if (saveBlockerAttached) return;
  saveBlockerAttached = true;
...
async function renderViewer(container, path) {
  attachSaveBlocker();
  container.innerHTML = `
    <div style="text-align:center; color:#aaa; padding:24px; font-family:-apple-system,sans-serif;">Loading PDF...</div>
  `;

  const url = new URL('../' + path, window.location.href).href;

  try {
    const blob = await getCachedOrFetchPdfBlob(url);
    const pdfData = await blob.arrayBuffer();
    const pdfDoc = await pdfjsLib.getDocument({ data: pdfData }).promise;

    container.innerHTML = '';
    const wrapper = document.createElement('div');
    wrapper.style.cssText = 'max-width:100%; overflow-x:hidden;';
    ...
    for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum++) {
      ...
      canvas.style.cssText = 'width:100%; height:auto; display:block; margin:0 auto 10px auto; -webkit-user-select:none; user-select:none;';
      // Blocks the casual "save image" / "open in..." tap on each page.
    ...
    // Rendering failed — most likely a stale/corrupt cache entry. Clear
    // that one cache entry so the NEXT attempt re-fetches clean, and tell
    // the user plainly rather than leaving a blank screen.
    try {
      const cache = await caches.open(PDF_CACHE_NAME);
    ...
    container.innerHTML = `
      <div style="background:#161616; border-radius:12px; padding:30px 20px; text-align:center; color:#eee; font-family:-apple-system,sans-serif;">
        ...
        <h3 style="color:#ffb703; margin:0 0 10px 0;">Couldn't load this PDF</h3>
        <p style="color:#aaa; margin:0;">Check your connection and reopen this page.</p>
      </div>
    `;
  }
}

// ---- Lock-state screens (unchanged) ----
