...
(function () {
  ...
  var MIN_ZOOM = 1;
  var MAX_ZOOM = 3;
  var ZOOM_SENSITIVITY = 0.45; // <1 slows the zoom down relative to finger movement
  ...
  rotateBtn.className = 'pdfv-rotate-btn';
  ...
  var pageCount = 0;
  var pageDivs = [];
  var pageHeightPx = 0; // uniform page height at zoom 1 (assumes consistent page size in the doc)
  var pageGap = 10;
  var baseWidth = 0;
  var zoomLevel = 1;
  var pageRotation = 0; // 0, 90, 180, 270
  ...
  loadScript(PDFJS_BASE + 'pdf.min.js')
    ...
    .then(function (firstPage) {
      ...
      // some browsers block fullscreen unless it's requested inside a
      // real user gesture — if the attempt above got refused, the very
      // next tap on the reader will retry once
      scroller.addEventListener('pointerdown', requestFullscreenSafe, { once: true });
  ...
  function buildPageSlots(firstPage) {
    var vp1 = firstPage.getViewport({ scale: 1, rotation: pageRotation });
  ...
  function renderPage(num) {
    ...
    pdfDoc.getPage(num).then(function (page) {
      var vp1 = page.getViewport({ scale: 1, rotation: pageRotation });
      var fitScale = baseWidth / vp1.width;
      ...
      var viewport = page.getViewport({ scale: renderScale, rotation: pageRotation });
  ...
  function attachGestures() {
    ...
    scroller.addEventListener('pointermove', function (e) {
      ...
      if (ids.length === 2 && gestureActive) {
        ...
        liveScale = 1 + (rawRatio - 1) * ZOOM_SENSITIVITY;
        var preview = clamp(startZoom * liveScale, MIN_ZOOM, MAX_ZOOM) / zoomLevel;
        pagesWrap.style.transform = 'scale(' + preview + ')';
        pagesWrap.style.transformOrigin = 'top center';
    ...
    function endPointer(e) {
      ...
      if (ids.length < 2 && gestureActive) {
        gestureActive = false;
        commitZoom(clamp(startZoom * liveScale, MIN_ZOOM, MAX_ZOOM));
        // one finger is still down after the pinch ends — start panning
        // from right here instead of waiting for a fresh pointerdown,
        // otherwise horizontal/vertical drag tracking freezes until re-touch
        if (ids.length === 1 && zoomLevel > 1.02) {
          var remainingId = ids[0];
          pan = {
            x: pointers[remainingId].x,
            y: pointers[remainingId].y,
  ...
  function commitZoom(newZoom) {
    ...
    var fractionBefore = scroller.scrollHeight > 0 ? scroller.scrollTop / scroller.scrollHeight : 0;
    zoomLevel = newZoom;
    var w = Math.round(baseWidth * zoomLevel);
    pageDivs.forEach(function (div) { div.style.width = w + 'px'; });
    // let layout settle, then restore proportional position
    requestAnimationFrame(function () {
      scroller.scrollTop = fractionBefore * scroller.scrollHeight;
      scroller.style.touchAction = zoomLevel > 1.02 ? 'none' : 'pan-y';
    });
  }

  // ---------- rotate ----------
  function attachRotateButton() {
  ...
  function rotate() {
    if (!pdfDoc) return;
    var stayOnPage = currentPage();
    pageRotation = (pageRotation + 90) % 360;

    pdfDoc.getPage(1).then(function (firstPage) {
      // zoom is reset because the page's fit geometry changes with rotation
      zoomLevel = 1;
      var vp1 = firstPage.getViewport({ scale: 1, rotation: pageRotation });
      baseWidth = Math.max(scroller.clientWidth - 4, 280);
      var fitScale = baseWidth / vp1.width;
      ...
      var toRerender = [];
      pageDivs.forEach(function (div, idx) {
        var wasRendered = !!div.dataset.rendered;
        div.style.width = baseWidth + 'px';
        div.style.height = pageHeightPx + 'px';
        div.innerHTML = '';
        delete div.dataset.rendered;
        if (wasRendered) toRerender.push(idx + 1);
      });

      scroller.style.touchAction = 'pan-y';
      scroller.scrollTop = (stayOnPage - 1) * (pageHeightPx + pageGap);
      toRerender.forEach(renderPage);
      flashHud(pageRotation === 0 ? 'Rotation reset' : 'Rotated ' + pageRotation + '\u00B0');
    });
  }
  ...
  function injectStyles() {
    ...
    style.textContent =
      '.pdfv-root{position:relative;width:100%;height:100%;min-height:0;background:#0b0704;' +
      ...
      '.pdfv-rotate-btn{position:fixed;top:14px;right:14px;width:38px;height:38px;' +
      'border-radius:50%;border:1px solid rgba(255,153,51,0.4);' +
      'background:rgba(15,8,3,0.75);color:#ffb066;font-size:18px;line-height:1;' +
      'display:flex;align-items:center;justify-content:center;cursor:pointer;' +
      'z-index:6;padding:0;-webkit-tap-highlight-color:transparent;}' +
      '.pdfv-rotate-btn:active{background:rgba(255,153,51,0.25);}' +
