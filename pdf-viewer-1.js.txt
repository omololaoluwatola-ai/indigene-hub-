...
(function () {
  ...
  // Everything heavy — the pdf.js library, its worker, and the PDF file
  // itself — gets stored here on first open. Every open after that is
  // served straight from here with zero network traffic: no data spent,
  // works with no connection at all. This is the Cache Storage API, not
  // the ordinary browser HTTP cache — it's explicit, under our control,
  // and isn't silently evicted the way an HTTP cache can be.
  var ASSET_CACHE = 'pdfv-assets-v1';
  ...
  loadScript(PDFJS_BASE + 'pdf.min.js')
    .then(function () {
      return cachedFetch(PDFJS_BASE + 'pdf.worker.min.js');
    })
    .then(function (res) {
      return res.blob();
    })
    .then(function (workerBlob) {
      // a blob URL, not the live CDN URL — so spinning up the worker
      // never touches the network once this has been cached once
      window.pdfjsLib.GlobalWorkerOptions.workerSrc = URL.createObjectURL(workerBlob);
      return fetchPdfWithProgress(pdfUrl);
    })
    .then(function (pdfArrayBuffer) {
      var loadingTask = window.pdfjsLib.getDocument({ data: pdfArrayBuffer });
  ...
  function reflowToViewport() {
    ...
    var oldBaseWidth = baseWidth || 1;
    baseWidth = Math.max(scroller.clientWidth - 4, 280);
    // each page's aspect ratio is fixed — its stored "natural" height
    // has to scale by the same ratio the width just changed by (e.g.
    // rotating to landscape roughly doubles the available width), or the
    // canvas inside it — which always fills its div at width:100%/
    // height:100% — stretches out of its correct proportions instead of
    // just getting bigger. zoomLevel itself is untouched here, so the
    // zoom the reader had (1x, 2x, whatever) stays exactly what it was.
    var widthRatio = baseWidth / oldBaseWidth;
    pageBaseHeights = pageBaseHeights.map(function (h) { return h * widthRatio; });
    pageHeightPx = pageHeightPx * widthRatio;
  ...
  function attachGestures() {
    ...
    var anchorPageIndex = 0, anchorPageFrac = 0;
    var pan = null;

    // pagesWrap has fixed, non-scaling gaps in it (10px top padding, 40px
    // bottom padding, 10px margin between every page) — only the pages
    // themselves grow with zoom. So "offset * ratio" is only exact for a
    // point on page 1 at y=0; anywhere else it drifts by every fixed gap
    // above the anchor, worse the more you zoom. Instead we record WHICH
    // page the anchor sits on and how far through it (both zoom-invariant),
    // then after the real resize we re-read that page's actual new
    // position from the DOM — exact regardless of how many fixed gaps
    // sit above it.
    function locatePageAnchor(y) {
      for (var i = 0; i < pageDivs.length; i++) {
        var div = pageDivs[i];
        var top = div.offsetTop;
        var h = div.offsetHeight || 1;
        if (y < top) return { index: i, frac: 0 };
        if (y <= top + h) return { index: i, frac: (y - top) / h };
      }
      var last = Math.max(pageDivs.length - 1, 0);
      return { index: last, frac: 1 };
    }
    ...
    scroller.addEventListener('pointerdown', function (e) {
      ...
      if (ids.length === 2) {
        ...
        var pageAnchor = locatePageAnchor(anchorContentY);
        anchorPageIndex = pageAnchor.index;
        anchorPageFrac = pageAnchor.frac;
    ...
    function endPointer(e) {
      ...
      if (ids.length < 2 && gestureActive) {
        ...
        commitZoom(newZoom, anchorContentX, anchorContentY, anchorScreenX, anchorScreenY, anchorPageIndex, anchorPageFrac);
      }
  ...
  function commitZoom(newZoom, anchorContentX, anchorContentY, anchorScreenX, anchorScreenY, anchorPageIndex, anchorPageFrac) {
    pagesWrap.style.transform = 'none';
    pagesWrap.style.transformOrigin = '';

    // X has no fixed, non-scaling gaps anywhere in it (pagesWrap has zero
    // left/right padding, pages are flush edge to edge), so a point at
    // pixel offset P scales to exactly P * (newZoom/oldZoom) — exact.
    ...
    requestAnimationFrame(function () {
      ...
      // Y is NOT a pure scale from the top — the top/bottom padding and
      // the gap between every page are fixed pixel values that don't
      // grow with zoom, so "offset * ratio" drifts off by every fixed
      // gap sitting above the anchor (worse the further down the doc and
      // the bigger the zoom jump — exactly the "deviates when I let go"
      // symptom). Re-read the anchor's own page after the real resize
      // above and rebuild its position from that page's actual new top
      // plus the same fraction through it — exact no matter how many
      // fixed gaps sit above it.
      var pd = pageDivs[anchorPageIndex] || pageDivs[0];
      var newContentY = pd ? (pd.offsetTop + anchorPageFrac * pd.offsetHeight) : (anchorContentY * ratio);
  ...
  function loadScript(src) {
    return cachedFetch(src)
      .then(function (res) {
        if (!res || !res.ok) throw new Error('script fetch failed: ' + src);
        return res.blob();
      })
      .then(function (blob) {
  ...
  // Cache-first fetch for the library/worker files: serves the stored
  // copy instantly (no network, no data used) on every visit after the
  // first, and only reaches out to the CDN when nothing's cached yet.
  // Falls back to a plain fetch on browsers with no Cache Storage API —
  // same as before, no regression there.
  function cachedFetch(url) {
    if (!hasCacheApi()) return fetch(url);
    return caches.open(ASSET_CACHE).then(function (cache) {
      return cache.match(url).then(function (hit) {
        if (hit) return hit;
        return fetch(url).then(function (res) {
          if (res && res.ok) {
            cache.put(url, res.clone()).catch(function () { /* storage full / private mode — reading still works, just won't be cached this time */ });
          }
          return res;
        });
      });
  ...
  // Same idea for the PDF itself, but also keeps the loading-bar
  // percentage working: a cache hit resolves instantly and a fresh
  // download is read in chunks against Content-Length like before.
  function fetchPdfWithProgress(url) {
    if (!hasCacheApi()) return fetch(url).then(readWithProgress);
    return caches.open(ASSET_CACHE).then(function (cache) {
      return cache.match(url).then(function (hit) {
        if (hit) {
          setLoadProgress(100);
        ...
        return fetch(url).then(function (res) {
          if (!res || !res.ok) throw new Error('pdf fetch failed: ' + url);
          cache.put(url, res.clone()).catch(function () { /* storage full / private mode — this open just won't be cached */ });
          return readWithProgress(res);
        });
      });
    });
  }
  ...
  function readWithProgress(res) {
    if (!res.body || !res.body.getReader) {
      // very old browser with no streams API — no progress to show, just wait
  ...
  function injectStyles() {
    ...
    style.textContent =
      // !important on every layout-critical property: every course page
      // defines its own "#pdf-container{position:relative;...}" rule, and
      // an ID selector always beats this .pdfv-root class selector on
      // specificity alone — without !important here, that per-page CSS
      // silently wins, the container never actually leaves the page's
      // flex layout, and the topbar/title stay visible above it instead
      // of the PDF filling the whole screen.
      '.pdfv-root{position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;height:100dvh!important;min-height:0!important;' +
